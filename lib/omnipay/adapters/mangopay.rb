    ...
    class Mangopay < Omnipay::Adapter
      ...
      def validate_payin(transaction_id)
        ...
        if payin['Status'] == 'SUCCEEDED'
          ...
          return payment_successful reference, transaction_id, amount
        else

          # Cancelation
          if ['101001', '101002'].include? payin['ResultCode']
            return payment_canceled(reference)

          # Failure
          else
            return payment_failed reference, "Refused payment for transaction #{transaction_id}.\nCode : #{payin['ResultCode']}\nMessage : #{payin['ResultMessage']}"
          end
        end
      end
